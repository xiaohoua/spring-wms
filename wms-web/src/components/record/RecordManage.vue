<template>
  <div>
    <div style="margin-bottom: 5px;">
      ...
      <el-select v-model="goodstypename" placeholder="请选择商品种类" style="margin-left: 5px">
        <el-option
    ...
    >
      ...
      <el-table-column prop="count" label="库存" width="60">
      </el-table-column>
      <el-table-column prop="storagename" label="仓库" width="60">
      </el-table-column>
  ...
  methods: {
    ...
    formatGoodstype(row){
      let temp = this.goodstypeData.find(item=>{//可以不用find用foreach匹配
        return item.id == row.goodstypename
      })
      return temp && temp.name
    },
    setType(myType){
